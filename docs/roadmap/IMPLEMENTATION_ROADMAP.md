...
- **Evidence (2026-08-05, application, HTTP and Prisma projection slice):** `packages/application/src/catalog/public-service.ts` enforces published-state projections, locale fallback and organization-scoped repository ports. `packages/db/src/repositories/public-catalog.ts` now provides organization-scoped published category/product queries, cursor pagination, latest published revision selection and bounded starting-price derivation. `apps/web/src/http/catalog.ts` provides category/product list/detail handlers with ETags and public stale-while-revalidate headers; tests cover locale projection, unpublished hiding, pagination, 304 replay and repository scoping. Full product-detail composition (variants, option groups, dimensions, assets and hotspots) remains for the next CAT-01 slice.
