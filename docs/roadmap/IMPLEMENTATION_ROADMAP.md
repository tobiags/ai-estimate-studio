...
- **Evidence (2026-08-05, application and HTTP projection slice):** `packages/application/src/catalog/public-service.ts` now enforces published-state projections, locale fallback and organization-scoped repository ports. `apps/web/src/http/catalog.ts` provides category/product list/detail handlers with bounded pagination, ETags and public stale-while-revalidate headers; tests cover locale projection, unpublished hiding, pagination and 304 replay. Prisma public-slug aggregate queries and full product-detail composition remain for the next CAT-01 slice.
