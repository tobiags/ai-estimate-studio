...
- **Evidence (2026-08-05, persistence and HTTP adapters):** `packages/db/src/repositories/pricing.ts` maps bounded rule documents, enforces organization scope, replaces draft rules transactionally with optimistic versions, and retires prior published revisions before publishing the selected set. `apps/web/src/http/pricing.ts` provides scoped get/update/simulate/publish handlers with CSRF hooks and RFC 7807 conflict/validation mapping; contracts and OpenAPI now require the validation document and mandatory publication scenarios. Repository and HTTP tests cover scoped reads, transactional draft replacement, conflicts and publication requests; audit/cache post-commit integration remains.
...
- **Evidence (2026-08-05, application, HTTP and Prisma projection slice):** `packages/application/src/catalog/public-service.ts` enforces published-state projections, locale fallback and organization-scoped repository ports; detail projections now cover revision text, variants, option groups/options, dimensions, assumptions, exclusions and viewer payloads. `packages/db/src/repositories/public-catalog.ts` provides organization-scoped published category/product queries, cursor pagination, latest published revision selection, bounded starting-price derivation and READY-asset viewer composition. `apps/web/src/http/catalog.ts` provides category/product list/detail handlers with ETags and public stale-while-revalidate headers; tests cover locale projection, detail composition, unpublished hiding, pagination, 304 replay and repository scoping. Catalog lifecycle commands, publication-window gates and concrete Next route bindings remain.
