...
- Playwright smoke passes on Chromium 151.0.7922.34 (Windows, desktop profile): initial render, pointer orbit, wheel zoom, analysis toggle, module removal/addition, live width/price update, language switch and client-side PDF download. Headless Chromium uses SwiftShader and the test deliberately avoids asserting GPU frame timing; visual baseline review remains a release checklist item.
